...
如果你要录一个 30 秒作品演示，建议按这个顺序：

1. 打开首页，先说“这不是按策略名堆列表，而是按交易假设筛结构”。
2. 在筛选区演示一次组合筛选，例如选 `看涨 + 波动率下行 + 卖时间 + 低风险`。
3. 让页面立刻收敛到几种候选结构，再点开一张策略卡，看 `最大亏损 / 保证金 / Greeks / 风险提醒`。
4. 最后打开对比矩阵，说“这个项目的重点是先帮助用户理解用途和风险，再决定是否深入做定价和回测。”。

一句话版本：

> 我做了一个以方向、波动率和时间价值为核心入口的期权策略助手，先帮助用户理解每种策略的用途和风险，再逐步扩展到更完整的期权决策工作流。
